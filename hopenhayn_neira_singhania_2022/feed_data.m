...
% Labor share
%------------------------------------------------------------------
% Dollar revenue for each firm size: p*exp(s)*n*^alpha, with n* = production
% labor (eqbm.nstar, the FOC-optimal labor demand excluding the fixed cost).
% The aggregate labor share below is the dollar wage bill w*(labor) divided
% by dollar revenue; the output price p and wage w appear explicitly, and the
% share is invariant to the overall (p,w) scale.
rev_firms = params.p .* exp(transpose(params.svec)) .* transpose(eqbm.nstar).^params.alpha ;
...
lshare_firms_l = params.w*transpose(eqbm.nstar_l)./rev_firms ; % wage bill / revenue
lshare_firms_h = params.w*transpose(eqbm.nstar_h)./rev_firms ; % wage bill / revenue
